...
def upload_image():
    ...
    if arquivo and arquivo.filename:
        ...
        caminho_disco = os.path.join(app.config['UPLOAD_FOLDER'], nome_seguro)
...
def api_dados_dashboard():
    ...
    apostas = Aposta.query.filter_by(usuario_id=usuario.id).order_by(Aposta.data.desc()).all()
    ...
    for a in apostas[:10]:
        ultimas.append({
            'data': a.data.strftime('%d/%m/%Y %H:%M') if a.data else '',
            'local': a.local,
            'valor': a.valor_final if a.resultado == 'Vitória' else a.valor,
...
def api_todas_apostas():
    ...
    # Inicializar dados por usuário com 0 (todas as linhas partem do mesmo ponto)
    dados_por_usuario = defaultdict(lambda: {
        'valores': [0] * len(posicoes),
    ...
    # Labels do eixo X
    labels = [posicoes[ident]['label'] for ident in sorted(
        posicoes.keys(), key=lambda x: posicoes[x]['posicao']
    )]

    # Cores para cada usuario
    cores = [
        '#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF',
        '#FF9F40', '#FFB1C1', '#9AD0F5', '#FF6B6B', '#4ECDC4',
        '#5B7D1',  '#96CEB4', '#FFEAA7', '#DDA0DD', '#98DBC8',
        '#7DC6F',  '#BB8FCE', '#85C1E9', '#F1948A', '#82E0AA'
    ]
...
def pagina_configuracoes():
    ...
    cookies = session.get('cookies', 'atividade')
...
def entrar_admin(pagina):
    ...
    if not usuario.vip.verificar_senha_vip(senha_digitada):
...
def pagina_ranking():
    ...
    for u in usuarios:
        total_apostado = db.session.query(func.sum(Aposta.valor)).filter(Aposta.usuario_id == u.id).scalar() or 0.0
        total_ganho = db.session.query(func.sum(Aposta.valor_final)).filter(
            Aposta.usuario_id == u.id,
            Aposta.resultado == 'Vitória'
        ).scalar() or 0.0
        vitorias = Aposta.query.filter_by(usuario_id=u.id, resultado='Vitória').count()
        ...
        ranking.append({
            ...
            'vitorias': vitorias,
            'saldo': u.saldo,
            'is_vip': u.vip is not None
        })

    # Ordena por total apostado (maiores)
    maiores_apostadores = sorted(ranking, key=lambda x: x['total_apostado'], reverse=True)[:10]
    # Ordena por total ganho (melhores)
    melhores_apostadores = sorted(ranking, key=lambda x: x['total_ganho'], reverse=True)[:10]

    return render_template('ranking.html',
                            maiores_apostadores=maiores_apostadores,
                            melhores_apostadores=melhores_apostadores)
...
def pagina_confirmar_admin_apostas():
    usuario = Usuario.query.get(session['usuario_id'])
    if not usuario.vip:
        flash('Acesso restrito a usuários VIP.', 'danger')
        return redirect(url_for('pagina_configuracoes'))
